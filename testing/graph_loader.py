...
class Graph:
    def __init__(self) -> None:
        ...
        self._edge_to:dict[str,list[tuple[Node,Edge]]] = {}
        self._edge_from:dict[str,list[tuple[Node,Edge]]] = {}
        self.nodes_dict = {}
    ...
    def add_edge(self, _from:Node, _to:Node, e:Edge):
        assert _from.label in self.node_set, f'{_from} not in nodes'
        assert _to.label in self.node_set, f'{_to} not in nodes'
        if (_from.label,_to.label) in self.edges:
            print("skipping")
            return
        self.edges[_from.label,_to.label] = e
        if not _to.label in self._edge_to:
            self._edge_to[_to.label] = []
        if not _from.label in self._edge_from:
            self._edge_from[_from.label] = []
        self._edge_to[_to.label].append((_from, e))
        self._edge_from[_from.label].append((_to, e))

    def edge_to(self, node:Node) -> list[tuple[Node,Edge]]:
        return self._edge_to.get(node.label, [])

    def edge_from(self, node:Node) -> list[tuple[Node,Edge]]:
        return self._edge_from.get(node.label,[])
...
def load_original_graph(file:str|TextIO) -> Graph:
    fp = file
...
def load_new_graph(file:str) -> Graph:
    fp = open(file)
    ...
    for line in fp.readlines():
        ...
        if nodes_lines:
            ...
            node_type = components[1]
            if node_type == 'literal_node':
                node = Node(label=label, _type=node_type, value=None)
            elif node_type == 'var_node':
                node = Node(label=label, _type=node_type, value=None)
            elif node_type == 'parameter_node':
                node = Node(label=label, _type='par_node', value=None)
            elif is_global(node_type):
                global_name = node_type.replace("_node","")
                node = Node(label=global_name + str(globals), _type=node_type)
                globals += 1
    ...
    for i in reversed(range(len(graph.nodes))):
        if len(graph.edge_from(graph.nodes[i])) == 0 and len(graph.edge_to(graph.nodes[i])) == 0:
            graph.nodes.pop(i)
