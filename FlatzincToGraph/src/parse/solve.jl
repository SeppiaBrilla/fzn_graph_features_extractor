...
function parse_solve(line::AbstractString, vars::IdDict{String, Variable})::SolveType
    ...
    elseif occursin("minimize", line)
        ...
        return SolveType("minimize", vars[intern(var_name)])
    elseif occursin("maximize", line)
        var_name = strip(replace(split(line, "maximize")[end], ";" => ""))
        return SolveType("maximize", vars[intern(var_name)])
    end
end
